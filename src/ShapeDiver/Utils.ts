...
import {
    getChunkNameFromAttributes,
    makeExampleSdtf,
    mapSdtfTypeHintToParameterType,
    parseSdtf,
    printSdtfInfo,
    readSdtf,
} from './SdtfUtils';
...
export const displayModelAccessData = async (
    identifier: string,
...
export const displayModelsByModelViewUrl = async (
    modelViewUrl: string,
    filename?: string
): Promise<void> => {
    const sdk = await initPlatformSdk();
...
export const fetchModelAnalytics = async (
    ...
    const modelsPerUser: IModelsPerUser = JSON.parse(
        await fsp.readFile(filename ?? analyticsExportFilename, { encoding: 'utf8' })
    );

    const sdk = await initPlatformSdk();

    for (const user_id in modelsPerUser) {
        const models = modelsPerUser[user_id].done.filter(
            (m) =>
                !m.analytics ||
                m.analytics.timestamp_from !== timestamp_from ||
                m.analytics.timestamp_to !== timestamp_to
        );
        ...
        for (const chunk of modelChunks) {
            ...
            for (const m of chunk) {
                try {
                    ...
                    if (!data.deleted_at) filteredChunk.push(m);
                    else console.log(`Model ${m.id} does not exist anymore.`);
                } catch (e) {
                    console.log(e);
            ...
            const data = await getAnalyticsAccessData(
                sdk,
                filteredChunk.map((c) => c.id),
                filteredChunk.map((c) => c.guid)
            );
...
export const notifyUsersAboutDecommissioning = async (filename?: string): Promise<void> => {
    ...
    const headingHtml = (level: number, heading: string) =>
        `<h${level}>${htmlEscape(heading)}</h${level}>`;
    const linkHtml = (url: string, text: string) =>
        `<a href="${url}" target="_blank">${htmlEscape(text)}</a>`;

    const htmlHeader =
        '<head><title>ShapeDiver - Rhino 5 Geometry Backend Decommissioning</title></head>';
    let htmlBody = headingHtml(1, 'ShapeDiver - Rhino 5 Geometry Backend Decommissioning');
    htmlBody +=
        'After almost 7 years of operating a ShapeDiver Geometry Backend system based on Rhino 5, we are planning to decommission it on 31 August 2023 (2023/08/31). ';
    htmlBody +=
        'Rhino 5 has not been updated since a long time, and almost no traffic is handled by this system anymore, which causes its operation to become uneconomic. ';
    htmlBody +=
        '<br/>Below please find a list of the ShapeDiver models owned by your account which are still operated on this system. ';
    ...
    htmlBody +=
        '<br/>Uploading your Grasshopper models to one of the new ShapeDiver Geometry Backend systems (operated using Rhino 6 or 7) will work flawlessly in many cases, although this is not guaranteed due to subtle differences in the behavior of Grasshopper. ';
    htmlBody +=
        '<br/>Your models hosted on the Rhino 5 Geometry Backend system will disappear from your dashboard on 1 September 2023 (2023/09/01). ';
    ...
    const listModelsHtml = (heading: string, models: IModelInfo[], type?: string): string => {
        ...
        models.map((m) => {
            html +=
                'Title: ' +
                linkHtml(`https://shapediver.com/app/m/${m.slug}`, m.title) +
                `, slug: ${m.slug}`;
            if (type) html += `, usage count: ${m.analytics.sessions[type]}`;
            html += '<br/>';
        });
        return html;
    };
    ...
    for (const user_id in modelsPerUser) {
        ...
        const modelsBackend = modelsPerUser[user_id].done.filter(
            (m) => m.analytics.sessions.backend > 0
        );
        const modelsDesktop = modelsPerUser[user_id].done.filter(
            (m) => m.analytics.sessions.desktop > 0
        );
        const modelsEmbedded = modelsPerUser[user_id].done.filter(
            (m) => m.analytics.sessions.embedded > 0
        );
        ...
        if (
            modelsApp.length > 0 ||
            modelsBackend.length > 0 ||
            modelsDesktop.length > 0 ||
            modelsEmbedded.length > 0
        ) {
            htmlUser +=
                'The following models hosted on the Rhino 5 Geometry Backend system and owned by you have been accessed in the past two months. The usage count shows how many sessions have been opened. ';
            ...
            htmlUser +=
                'None of your models hosted on the Rhino 5 Geometry Backend system have been accessed in the past three months.';
        }
        ...
        if (email.endsWith('shapediver.com')) await createNotification(user_id, href);
    }
};
...
const dayTimestampToEpoch = (ts: string): number => {
    if (ts.length != 8) throw new Error('Provide a timestamp in format YYYYMMDD, e.g. 20220815');
    let num = 0;
    try {
...
export const displayUserCreditUsage = async (
    identifier: string,
    days: number,
    from_s: string,
    to_s: string
...
export const sdTFExample = async (
    identifier: string,
    sdTFfilename?: string,
    saveSdtfs?: boolean
): Promise<void> => {
    ...
    if (sdTFfilename) {
        ...
        sdTFbuffer = await makeExampleSdtf([
            SdtfTypeHintName.STRING,
            SdtfTypeHintName.GEOMETRY_LINE,
            SdtfTypeHintName.GEOMETRY_POLYLINE,
            SdtfTypeHintName.RHINO_CURVE,
            SdtfTypeHintName.GEOMETRY_POINT,
        ]);
    ...
    for (const chunk of sdTFasset.chunks) {
        ...
        const chunkDisplayName = chunkFriendlyName
            ? `id "${chunkId}" name "${chunkFriendlyName}"`
            : `id "${chunkId}"`;
        ...
        if (!parameterType) {
            console.warn(
                `Skipping chunk ${chunkDisplayName} with typeHint "${typeHint}" for which no matching parameter type was found.`
            );
            continue;
        }
        ...
        if (params.length === 0) {
            console.warn(
                `No matching parameter for chunk ${chunkDisplayName} with typeHint "${typeHint}".`
            );
            continue;
        } else if (params.length > 1) {
            console.warn(
                `Multiple matching parameters for chunk ${chunkDisplayName} with typeHint "${typeHint}", picking the first one.`
            );
        }
        const param = params.find((p) => !requestDto[p.id]);
        if (!param) {
            console.log(
                `Skipping chunk ${chunkDisplayName} with typeHint "${typeHint}" (parameters already matched to other chunks).`
            );
        } else {
            requestDto[param.id] = {
            ...
            console.log(
                `Matched chunk ${chunkDisplayName} with typeHint "${typeHint}" to parameter id "${param.id}" name "${param.name}" with type "${param.type}".`
            );
        }
    }
...
export const notifyUsersPlatform = async (
    options: NotifyUsersUserOptions,
    notification_options: NotifyUsersNotificationOptions
) => {
    const sdk = await initPlatformSdk();
