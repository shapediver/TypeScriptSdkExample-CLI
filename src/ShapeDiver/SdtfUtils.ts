 ...
 * @returns
 */
export const makeExampleSdtf = async (
    chunkTypes: Array<SdtfTypeHintName>
): Promise<ArrayBuffer> => {
    ...
    if (chunkTypes.includes(SdtfTypeHintName.STRING)) {
        ...
        builder.addChunkForTreeData(
            'String',
            { branches: branches, paths: paths },
            factory.createAttributes({ Name: ['Text', SdtfPrimitiveTypeHintName.STRING] })
        );
    }
    ...
    if (chunkTypes.includes(SdtfTypeHintName.GEOMETRY_LINE)) {
        ...
        const line1: SdtfGeometryLineType = [
            [0, 0, 0],
            [1, 0, 0],
        ];
        const line2: SdtfGeometryLineType = [
            [1, 0, 0],
            [2, 1, 0],
        ];
        const line3: SdtfGeometryLineType = [
            [2, 1, 0],
            [1, 2, 0],
        ];
        ...
        builder.addChunkForTreeData(
            'Line',
            { branches: branches, paths: paths },
            factory.createAttributes({ Name: ['Line', SdtfPrimitiveTypeHintName.STRING] })
        );
    }
    ...
    if (chunkTypes.includes(SdtfTypeHintName.GEOMETRY_POLYLINE)) {
        ...
        builder.addChunkForTreeData(
            'Polyline',
            { branches: branches, paths: paths },
            factory.createAttributes({ Name: ['Polyline', SdtfPrimitiveTypeHintName.STRING] })
        );
    }
    ...
    if (chunkTypes.includes(SdtfTypeHintName.RHINO_CURVE)) {
        ...
        builder.addChunkForTreeData(
            'Curve',
            { branches: branches, paths: paths },
            factory.createAttributes({ Name: ['Crv', SdtfPrimitiveTypeHintName.STRING] })
        );
    }
    ...
    if (chunkTypes.includes(SdtfTypeHintName.GEOMETRY_POINT)) {
        ...
        builder.addChunkForTreeData(
            'Point',
            { branches: branches, paths: paths },
            factory.createAttributes({ Name: ['Pt', SdtfPrimitiveTypeHintName.STRING] })
        );
    }
 ...
 * @param buffer
 */
export const readSdtf = async (
    buffer: ArrayBuffer | string,
    authToken?: string
): Promise<ISdtfReadableAsset> => {
    // create an instance of the sdTF SDK, also using the Rhino3dm integration
    const sdk = await createSdtfSdk({
    ...
    if ((buffer as string).padStart) {
        ...
        if (str.startsWith('http')) asset = await parser.readFromUrl(str);
        else asset = await parser.readFromFile(buffer as string);
    } else {
 ...
 * @param chunk
 */
export const getChunkNameFromAttributes = async (
