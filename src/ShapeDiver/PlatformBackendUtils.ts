 ...
 * @returns
 */
export const getModelAccessData = async (
    sdk: SdPlatformSdk,
    identifier: string,
    ...
    const model = (
        await sdk.models.get<SdPlatformResponseModelOwner>(identifier, [
            backend
 ...
 * @returns
 */
export const getAnalyticsAccessData = async (
    sdk: SdPlatformSdk,
    id: string | string[],
    guid: string | string[]
): Promise<IGeometryBackendAccessData> => {
 ...
 * @returns
 */
export const listLatestModels = async (
    sdk: SdPlatformSdk,
    limit: number,
    own: boolean
): Promise<SdPlatformResponseModelPublic[]> => {
    const filters = {
        'status[=]': 'done',
    };
    if (own) {
 ...
 * @returns
 */
export const queryAllMatchingModels = async (
    sdk: SdPlatformSdk,
    filters: any,
    ...
    while (true) {
        const result = (
            await sdk.models.query<SdPlatformResponseModelOwner>({
                sorters: { created_at: SdPlatformSortingOrder.Desc },
                limit: 100,
                filters,
                embed: [SdPlatformModelQueryEmbeddableFields.User],
        ...
        if (!result.pagination.next_offset) break;
...
export const createModel = async (
    ...
    const body: SdPlatformRequestModelCreate = {
        filename,
        ftype: filename.toLocaleLowerCase().endsWith('.ghx')
    ...
    const scopes = [SdPlatformModelTokenScopes.GroupOwner, SdPlatformModelTokenScopes.GroupView];
 ...
 * @returns
 */
export const patchModelStatus = async (
    sdk: SdPlatformSdk,
    model_id: string,
 ...
 * @returns
 */
export const queryUserCreditUsage = async (
    sdk: SdPlatformSdk,
    user_id: string,
    from: number,
    to: number,
    type: SdPlatformResponseAnalyticsTimestampType = SdPlatformResponseAnalyticsTimestampType.Day
...
export const notifyUsers = async (
    sdk: SdPlatformSdk,
    notify_users_user_options: NotifyUsersUserOptions,
    notification_options: NotifyUsersNotificationOptions
): Promise<Array<SdPlatformResponseUserPublic>> => {
    const uo = notify_users_user_options;

    // validation
    if (
        -1 ===
        Object.keys(SdPlatformNotificationType).findIndex(
            (k) => SdPlatformNotificationType[k] === notification_options.type
        )
    ) {
        throw new Error(
            `Notification type must be one of ${Object.values(SdPlatformNotificationType).join(',')}`
        );
    }
        ...
        if (uo.dry_run === false) {
            ...
            for (let i = 0; i < users.length; i += chunkLength) {
                ...
                for (const user of chunk)
                    console.log(
                        `Creating notification for user id "${user.id}", username "${user.username}", email "${user.email}".`
                    );
